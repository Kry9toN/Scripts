...
git add . && git cherry-pick --continue"
        ...
        "merge")
            if ! GIT_MERGE_VERBOSITY=1 git merge "v${TARGET_VERSION}" -m "Merge v${TARGET_VERSION} into $(git branch | grep "\*" | sed 's/\* //')

$(git log --oneline $(git branch | grep "\*" | sed 's/\* //')..v${TARGET_VERSION})" --signoff; then
